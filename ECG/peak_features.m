...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% peak_features.m 
% detection, localisation des QRS, onde R et mesure des parametres caracteristiques QRS width
% QRS onset, QRS offset
% 
% INPUTS :
%
% preproc_sig : signal pre-traite sur lequel on va detecter et pre-localiser les QRS
% b1,b2,b3,b4,b5,b6 : coefficients des filtres utilisés pour la pre-localisation des QRS
% ces coefficients vont permettre de calculer le "retard" entre le signal pre_traité (preproc_sig) et le signal brut (ecg)
% cela nous permettra en particulier de recaler les QRS pre-localises, pour la localisation plus fine sur le signal brut
%       b1: numerateur du filtre passe bas
%                   b2: numerateur du passe pas utilé dans la formation du passe haut
%                   b3: numerateur du passe tout
%                   %% rem ces deux vecteur viennent de l'implantation du passe haut= all_pass - low_pass
%                   %% si on definit directement un passe haut alors b2=b_passehaut et b3=[]; 
%       b4 : numerateur du derivateur
%       b5 : numerateur de la fenetre glissante
% ecg : Signal brut (normalisé), sera utilisé pour la localisation precise et l'extraction de parametres complementaire (QRS width)
% Fs : fréquence d'échantillonnage
% 
% OUTPUTS :
%
% QRS_loc : vecteur contenant les positions (num echantillon) des QRS detectes et localisés
% QRS_width_buff : vecteur contenant les largeurs des QRS detectes
% QRS_buff_on : vecteur contenant le debut des QRS detectes (marquage debut onde Q)
% QRS_buff_off : vecteur contenant la fin des QRS detectes (marquage fin onde S)
%
% remarque : QRS_width_buff=QRS_buff_off-QRS_buff_on

%% calcul du decalage induit par le passe bande
bpb=[b1, b2, b3];

[signal_recherche,loc_peak,recal]=marque(bpb,b4,b5,preproc_sig,Fs);

% figure
% plot(signal_recherche)
% hold on
% stem(loc_peak, signal_recherche(loc_peak))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Sur le signal brut
%% la detection des QRS a fournit une localisation approximative
%% des R waves (a partir du signal op)

%% Tout d'abord : segmentation large d'un battement a partir des localisations
%% approchees puis recherche d'un couple min-max sur la derivee de ce segment et enfin
%% on recherche le passage a zero entre ce min-max (annulation de la derivee donne
%% un max
...
% plot(ecg-mean(ecg))
% hold on
% stem(QRS_loc, ecg(QRS_loc)-mean(ecg),'g')
...
%%*  
%%**************************************************************  
%% Sur le signal brut, decoupage du complexe QRS, debut QRS, fin QRS
%% et par consequent estimation de la largeur du QRS

%% Comme pour la localisation excate de l'onde R, on utilise la derivee
%% du battement decoupé; on cherche les valeurs pour lesquelles cette derivee
%% devient tres faible a l'exterieur du min-max precedent

[QRS_width_buff,QRS_buff_on,QRS_buff_off]=QRSestim(QRS_loc,ecg,Fs);

% hold on
% stem(QRS_buff_on, ecg(QRS_buff_on)-mean(ecg),'r')
% stem(QRS_buff_off, ecg(QRS_buff_off)-mean(ecg),'k')
% xlabel('samples')
% ylabel('Amplitude')
% title('R waves Fiducial marks and  QRS onset and offset delineation')

% figure
% plot(QRS_width_buff/Fs)
% xlabel('n° of the detected QRS')
% ylabel('QRS width (in sec)')
% title('QRS width of each detected QRS')

figure;
hold on;
xlabel('echantillon');
plot(ecg/8);title('correspondance pic detecté / signal');
stem(QRS_buff_on,-4*( ecg(QRS_buff_on)-mean(ecg)),'r');
hold off; 

%% Remarque : toujours garder a l'esprit que la resolution temporelle est de 1/FS=4ms
%% pour l'(ensemble des grandeurs calculees
...
%
% INPUTS
% b1,b2,b3 : coefficients des filtres 
% b1 : passe bande
% b2 : dérivateur
% b3 : moyenne glissante
% op : signal à marquer
% Fs : fréquence d'échantillonnage
%
% OUTPUTS
% signal_recherche : Signal à marquer (signal pre-traite)
% loc_peak : indice des pics
% Ndeb : nombre d'echantillons abandonés en debut d'analyse (signal filtre non stable)
...
%% limite temporelle entre 2 pics
MS200=round(0.2*Fs);
MS300=round(0.3*Fs);
fact=0.3;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
%% Recherche des marker points correspondant au R detectes
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% INPUTS
%
% b1,b2,b3,b4,b5 : ordres des filtres pour le calcul du retard sur la localisation issue de marquage
% loc_peak : buffer contenant les pre-localisations issues de marquage
% recal : longueur de signal eliminee lors de la detection des QRS (issue de marquage)
% ecg : signal brut (normalisé)
% Fs : fréquence d'échantillonnage
%
% OUTPUTS
%
% QRS_loc : buffer contenant les localisation exacte des QRS detectes
 
function [QRS_loc]=Rwavexact(b1,b2,b3,b4,b5,loc_peak,recal,necg,Fs)

%% On va utiliser la derivée pour localiser exactement ces max
%% Recalage par rapport au signal initial
FILTER_DELAY=fix((length(b1)-1)/2)+fix((length(b2)-1)/2)+fix((length(b3)-1)/2)+fix((length(b4)-1)/2)+fix((length(b5)-1)/2);
%% la localisation des maxima obtenu par marquage est recentre
%% en fonction du nombre de points en debut de signal "elimines" (recal) dans la procedure marquage
%% et en prenant en compte les retards introduits par les differents filtres
...
%% On recherche dans un voisinage de cette loc les max et min de la derivee
...
%% Pre_proc : noise filtering using dyadic wavelet
%% Attention, si le signal n'est pas assez long on ne peut pas le decomposer jusqu'au
%% niveau 8 (par defaut pour l'elimin de la baseline) donc pas de baseline removing
